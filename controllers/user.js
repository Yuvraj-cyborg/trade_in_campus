...
async function addUser(req, res) {
  try {
    ...
    let user = await User.findOne({
      $or: [{ mobile: mobile }, { email: email }, { sic: sic }],
    });
    if (user) {
...
async function getUserBySic(req, res) {
  try {
    ...
    res
      .status(400)
      .send({ message: "Error searching by SIC", error: error.message });
  }
}
async function updateUser(req, res) {
  try {
    ...
    updatedUser = await User.findOneAndUpdate({ _id: id }, updatedUser, {
...
export { addUser, allUsers, getUserById, getUserBySic, updateUser, deleteUser };
